...
function setup_global_keyboard_shortcuts(){
  $('body').keydown(function(event){
    var target = $(event.target);
    var tag = target.attr('tagName');
    var editing = target.is(':input') || tag == 'TEXTAREA' || tag == 'SELECT';
    if (editing) {
      return true;
    }

    if (event.which == 191) {
      var inline_filter = $('#inline_filter_input:visible');
      if (inline_filter.size() > 0) {
        inline_filter.focus().select();
      }
      else {
    ...
    if (event.which == 27) {
      var filter = $('#inline_filter_input:visible');
      if (filter.size() > 0 && filter.val().length > 0) {
        filter.val('');
        apply_inline_list_filter();
        event.preventDefault();
        return false;
      }
    }
...
function setup_inline_list_filter(){
  var table = $('#maincontents table').filter(function(){
    return $(this).find('tr.selectable,tr.unselectable').size() > 10;
  }).eq(0);

  if (table.size() < 1 || $('#inline_filter').size() > 0) {
    return;
  }

  var filter = $('<div id="inline_filter" class="inline_filter"></div>');
  filter.append('<label for="inline_filter_input">ページ内絞り込み</label>');
  filter.append('<input id="inline_filter_input" type="text" autocomplete="off" />');
  filter.append('<span id="inline_filter_count" class="inline_filter_count"></span>');
  filter.append('<button id="inline_filter_select" type="button">表示中を選択</button>');
  filter.append('<button id="inline_filter_clear" type="button">クリア</button>');

  table.before(filter);
  $('#inline_filter_input').keyup(function(){apply_inline_list_filter();});
  $('#inline_filter_clear').click(function(){
    $('#inline_filter_input').val('');
    apply_inline_list_filter();
    $('#inline_filter_input').focus();
    return false;
  });
  $('#inline_filter_select').click(function(){
    clear_selections();
    get_inline_filter_rows().filter(':visible').filter('.selectable').each(function(){
      select_on_selectable($(this));
    });
    return false;
  });
  apply_inline_list_filter();
...
function get_inline_filter_rows(){
  return $('#maincontents table').eq(0).find('tr.selectable,tr.unselectable');
}

function apply_inline_list_filter(){
  var query = $.trim($('#inline_filter_input').val() || '').toLowerCase();
  var rows = get_inline_filter_rows();
  var matched = 0;

  rows.each(function(){
    var row = $(this);
    var row_text = row.text().toLowerCase();
    var row_title = (row.attr('title') || '').toLowerCase();
    var visible = query.length < 1 || row_text.indexOf(query) >= 0 || row_title.indexOf(query) >= 0;
    row.toggle(visible);
    if (visible) {
      matched++;
    }
  ...
  var table = rows.eq(0).closest('table');
  if (query.length > 0) {
    table.find('tr.host_category_summary,tr.host_category_summary_total,tr.brick_category_summary,tr.brick_category_summary_total,tr.sortbar').hide();
  }
  else {
    table.find('tr.host_category_summary,tr.host_category_summary_total,tr.brick_category_summary,tr.brick_category_summary_total,tr.sortbar').show();
  }

  $('#inline_filter_count').text(matched + ' / ' + rows.size());
}
...
function set_smartadd_form_busy(form, busy) {
  ...
  if (busy) {
    target.find(':submit, :image, button').attr('disabled', 'disabled');
    target.find('input[type=text], textarea').attr('readonly', 'readonly');
  } else {
    target.find(':submit, :image, button').removeAttr('disabled');
    target.find('input[type=text], textarea').removeAttr('readonly');
  }
}
