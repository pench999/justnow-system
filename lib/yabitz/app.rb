...
if Yabitz.config.name == :development and Yabitz.config.ldapparams and not Yabitz.config.ldapparams.empty?
  ...
  Net::LDAP.new(:host => ldap[0], :port => ldap[1].to_i)
end
###########
...
class Yabitz::Application < Sinatra::Base
  ...
  configure do
    ...
    session_secret = ENV['YABITZ_SESSION_SECRET']
    unless session_secret and session_secret.size >= 32
      system_boot_str = ""
      open('|who -b') do |io|
        system_boot_str = io.readlines.join
      end
      session_secret = Digest::SHA1.hexdigest(system_boot_str)
    end
    use Rack::Session::Cookie, :expire_after => 3600*48, :secret => session_secret
  end
