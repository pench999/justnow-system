  ...
  module InstantMemberHandler
    ...
    def self.query(sql, *args)
      ...
      begin
        ...
        client.query(bind_sql(client, sql, args)).each(:as => :array) {|r|
    ...
    def self.bind_sql(client, sql, args)
      args.inject(sql.dup) do |bound_sql, value|
        quoted = value.nil? ? 'NULL' : "'#{client.escape(value.to_s)}'"
        bound_sql.sub('?', quoted)
      end
    end
