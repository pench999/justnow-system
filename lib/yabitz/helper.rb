...
module Sinatra
  ...
  module BrickCategorize
    def categorize_brick(bricks)
      result = {}
      Yabitz::Model::Brick::STATUS_LIST.each do |s|
        result[s] = {:list => [], :productname => {'不明' => 0}}
      end
      result['total'] = {:list => [], :productname => {'不明' => 0}}
      bricks.each do |brick|
        result[brick.status][:list].push(brick)
        result['total'][:list].push(brick)
        if brick.productname
          result[brick.status][:productname][brick.productname] ||= 0
          result[brick.status][:productname][brick.productname] += 1
          result['total'][:productname][brick.productname] ||= 0
          result['total'][:productname][brick.productname] += 1
        else
          result[brick.status][:productname]['不明'] += 1
          result['total'][:productname]['不明'] += 1
        end
      end
      result
    end
  end
  helpers BrickCategorize
