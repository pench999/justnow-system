...
- nginx Basic 認証プロキシと管理画面からのBasic認証ユーザー管理
- 読み取り専用 API v1 の追加
...
- ホスト一覧からラック図への遷移と対象ホストの強調表示
- ラック図の表示密度切り替え、空きU表示、ホスト種別/状態の色分け
- スマートフォン向けの参照用モバイル画面
- ハードウェア種別、OS 別の集計
- ホスト情報の欠損や矛盾チェック
- JSON/CSV 出力
...
- ホスト詳細メモは管理者のみ API で取得できます。
- API のホスト詳細には `can_view_notes` が含まれ、非管理者には `notes` が返りません。
...
デフォルトでは nginx の `web` サービス経由で、ホスト側の `127.0.0.1:9292` に公開されます。
...
`web` サービスは nginx Basic 認証をログイン入口に使用します。初回は `docker/nginx/.htpasswd` を作成してください。

例:

```bash
docker run --rm -it -v "$PWD/docker/nginx:/work" httpd:2.4-alpine htpasswd -cB /work/.htpasswd batchmaker
chmod 644 docker/nginx/.htpasswd
docker compose restart web
```

2人目以降を追加する場合は `-c` を外します。

```bash
docker run --rm -it -v "$PWD/docker/nginx:/work" httpd:2.4-alpine htpasswd -B /work/.htpasswd USERNAME
chmod 644 docker/nginx/.htpasswd
docker compose restart web
```
...
yabitz/JustNow System は、ホスト情報の登録・編集などに認証を要求します。現在の Docker Compose 構成では、nginx の Basic 認証でログイン入口を保護し、認証済みユーザー名をアプリへ渡します。

主な選択肢は以下です。

- nginx Basic 認証 + trusted proxy header
...
Basic認証ユーザーは、ADMIN 権限を持つユーザーでログイン後、「管理項目」→「Basic認証ユーザー」から追加、パスワード変更、削除できます。この画面は `docker/nginx/.htpasswd` を更新します。

ADMIN 権限を持つユーザーがまだ存在しない場合、ログイン済みユーザーを管理者として扱う互換動作があります。検証環境ではこの動作を利用できますが、本番運用では明示的に管理者を設定してください。

ホスト詳細のメモ欄は、閲覧・編集ともに ADMIN 権限を持つユーザーだけが利用できます。一般ユーザーはホスト詳細からメモ欄を参照できません。

アプリ本体の `app` サービスは Docker ネットワーク内だけに公開し、外部公開は `web` サービス経由にしてください。trusted proxy header 認証を使うため、`app` を直接外部公開しないでください。

## モバイル画面

参照用途向けのモバイル画面を用意しています。スマートフォンではホスト、サービス、ラック、IP セグメント、IP アドレスをカード形式で確認できます。

- 主用途は参照です
- ラック図は既存画面を利用します
- 詳細データは API v1 を利用して取得します
- ホスト詳細メモは管理者だけ表示されます
...
- `git pull` だけでは変更は反映されません
- `app` はイメージ build 時にソースを取り込むため、更新後は `docker compose build app` が必要です
