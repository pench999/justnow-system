...
# :sslcipher - Necessary to use MySQL with an SSL connection.
    ...
    def connect
      Mysql2::Client.new($STRATUM_CONNECTION_DBPARAMS.merge({:reconnect => true, :cast_booleans => false}))
    end
    ...
    def query(sql, *values)
      begin
        @handler.query(bind_sql(sql, values))
    ...
    def bind_sql(sql, values)
      ...
      values.inject(sql.dup) do |bound_sql, value|
        bound_sql.sub('?', quote(value))
      end
    end
    ...
    def method_missing(name, *args, &block)
      return super unless @handler.respond_to?(name)

      @handler.public_send(name, *args, &block)
    end

    def respond_to_missing?(name, include_private=false)
      @handler.respond_to?(name, include_private) || super
    end
  end
end
